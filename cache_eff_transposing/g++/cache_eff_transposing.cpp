...
template< class T > void simple_transposing_bucketing(T *Ain,T *Aout,int *permutationVector,int n,int buckets){
    ...
    if(buckets==1){
        #pragma omp parallel shared(Aout,Ain,permutationVector)
        {
        ...
        for(int j=0;j<buckets;j++){
            ...
            {
                #pragma omp for schedule(static)
                for(int i=j*sizeOfSubBuckets;i<(j+1)*sizeOfSubBuckets;i++){
                    memcpy(&Aout[permutationVector[i]],&Ain[i],sizeof(T));
                }
            }
        }
...
double improvedSerialReadFromInput(testingStruct *Ain, testingStruct *Aout, int *permutationVector,int n,int number_of_buckets,int* internalSubBucketsSizes){
    ...
    /**
     * Every bucket is splitted in number_of_buckets subbuckets and the data are sorted in order to be in correct places.
     */
    #pragma omp parallel shared(permutationVector,oneBucketElements,oneSubBucketElements,Ain,AIntermediate)
    {
        #pragma omp for schedule(static)
    ...
    //SECOND STAGE - Rearrange the sub buckets from all buckets to the correct bucket.

    #pragma omp parallel shared(AIntermediate,permutationVector,oneBucketElements,oneSubBucketElements)
    {
        #pragma omp for schedule(static)
    ...
    //THIRD STAGE - Apply the classic method to permutate every subbucket's ellements correctly.
